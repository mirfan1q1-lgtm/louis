...
export function StudentProfile() {
  const { student, refreshStudentSession } = useStudentAuth();
  ...
  const handleUpdateProfile = async (values: typeof form.values) => {
    ...
    try {
      ...
      const emailChanged = values.email !== student.email;
      ...
      if (emailChanged) {
        ...
        if (existingStudent) {
          throw new Error('Email sudah digunakan oleh siswa lain');
        }
      }
      ...
      if (values.address !== undefined) {
        updateData.address = values.address || null;
      }
      
      const updatedStudent = await studentAuthService.updateStudent(student.id, updateData);

      // Refresh student session to get updated data
      ...
      form.setValues({
        ...
        birth_date: dayjs(updatedStudent.birth_date).format('DD/MM/YYYY'),

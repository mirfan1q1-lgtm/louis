...
import { LoadingSpinner, EmptyState, ConfirmDialog, DisqusComments } from '../../components';
...
export function Newsroom() {
  useSEO({
    title: 'Newsroom',
    description: 'Newsroom LOUIS - Publikasikan berita dan pengumuman penting untuk siswa.',
    keywords: 'newsroom LOUIS, pengumuman sekolah, berita pendidikan, komunikasi sekolah',
  });
  const { user } = useAuth();
  const teacher = user!;
                ...
                <Box style={{ overflowX: 'auto', width: '100%' }}>
                  ...
                  <Table.Thead>
                    <Table.Tr>
                      ...
                      <Table.Th style={{ width: 120 }}>Aksi</Table.Th>
                    </Table.Tr>
                  </Table.Thead>
                  <Table.Tbody>
          ...
          {selectedItem && (
            <Stack gap="md">
              ...
              {selectedItem.status === 'published' && (
                <DisqusComments
                  shortname={import.meta.env.VITE_DISQUS_SHORTNAME || 'lms-newsroom'}
                  identifier={`newsroom-${selectedItem.id}`}
                  url={`${window.location.origin}/newsroom/${selectedItem.id}`}
                  title={selectedItem.title}
                  language="id"
                />
              )}

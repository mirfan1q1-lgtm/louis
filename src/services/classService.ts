...
export const classService = {
  async getAllClasses(isActive: boolean = true) {
    ...
    return data?.map(cls => ({
      ...cls,
      student_count: Array.isArray(cls.class_students) 
        ? (cls.class_students[0]?.count || 0)
        : (typeof cls.class_students === 'object' && cls.class_students !== null && 'count' in cls.class_students
          ? cls.class_students.count
          : cls.class_students || 0)
    })) || [];
  },
  ...
  async moveStudent(studentId: string, fromClassId: string, toClassId: string, movedBy: string) {
    // First, unenroll from old class
    await this.unenrollStudent(fromClassId, studentId);
    
    // Then, enroll to new class
    const enrollment = await this.enrollStudent(toClassId, studentId, movedBy);
    
    // Get class names for logging
    const [fromClass, toClass] = await Promise.all([
      supabase.from('classes').select('name').eq('id', fromClassId).single(),
      supabase.from('classes').select('name').eq('id', toClassId).single(),
    ]);
    ...
    await supabase.from('activity_logs').insert({
      ...
      entity_type: 'student',
      entity_id: studentId,
      description: `Moved student from ${fromClass.data?.name || 'unknown'} to ${toClass.data?.name || 'unknown'}`,
    });

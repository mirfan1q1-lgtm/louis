...
export const studentService = {
  async getAllStudents(isActive: boolean = true) {
    ...
    // Transform the data to extract class_count properly
    return data?.map(student => ({
      ...student,
      class_count: Array.isArray(student.class_count) 
        ? (student.class_count[0]?.count || 0)
        : (typeof student.class_count === 'object' && student.class_count !== null && 'count' in student.class_count
          ? student.class_count.count
          : student.class_count || 0)
    })) || [];
  },
  ...
  async bulkImportStudents(
    ...
    for (const studentData of students) {
      try {
        ...
        const defaultPassword = studentData.email.split('@')[0] + '123';
        const student = await this.createStudent(studentData, defaultPassword, createdBy);
